...
from urllib.urequests import pathname2url
...
__all__ = [
    ...
    'SKOS', 'DOAP', 'FOAF', 'DC', 'DCTERMS', 'VOID', 'SSN', 'SOSA']
...
class NamespaceManager(object):
    ...
    def compute_qname(self, uri, generate=True):
        ...
        if not uri in self.__cache:
            ...
            if prefix is None:
                if not generate:
                    raise Exception(
                        "No known prefix for %s and generate=False", namespace)
                num = 1
                while 1:
                    prefix = "ns%s" % num
    ...
    def absolutize(self, uri, defrag=1):
        ##FIXME base = Path.cwd().as_uri()
        ##base=""
        ##base = urljoin("file:", pathname2url(uos.getcwd()))
        print("Warning namespace.py absolutize: getcwd is different on linux vs other platforms")
        base = urljoin("file:", pathname2url('./'))
...
##FIXME For the moment just searching for / or #
def split_uri(uri):
